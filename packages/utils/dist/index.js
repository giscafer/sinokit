...
function logger() {
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (args[0] && args[0] instanceof Vue && args[0].$vnode['componentOptions']) {
      var _console;

      (_console = console).log.apply(_console, ["component[".concat(args[0].$vnode['componentOptions']['tag'], "]:")].concat(_toConsumableArray(args.slice(1))));
    } else {
      var _console2;

      (_console2 = console).log.apply(_console2, ["LOG>>>"].concat(args));
    }
  }
}
...
export { CustomEventPlugin, errorBox, errorToast, infoToast, logger, showToast, successToast, warningToast };
